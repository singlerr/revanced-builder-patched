...
async function downloadApp(ws) {
  ...
  let versionDownload = await fetchCompatible(
  ...
  const downloadLinkPage = await fetchCompatible(
  ...
  const downloadPage = await fetchCompatible(`https://www.apkmirror.com${pageLink}`).then(
    (res) => res.text()
  );
